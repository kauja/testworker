...
function runOriginFromEnv(): CrawlOptions['runOrigin'] | undefined {
  const raw = process.env.RUN_ORIGIN;
  if (!raw) return undefined;
  const parsed = CrawlOptions.shape.runOrigin.safeParse(raw);
